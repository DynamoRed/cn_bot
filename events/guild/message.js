...
module.exports = async (bot, message) => {
    ...
    if(message.content.toLowerCase() == bot.config.SECRET_PHRASE){
        message.delete();
        bot.db.query(`SELECT * FROM discord_badges WHERE badge_owner='${message.author.id}' AND badge_name='secret_phrase_v2'`, async function(err, results){
            if (err) throw err;
            if(results == undefined || results.length == 0){
                ...
                bot.db.query(`INSERT INTO discord_badges (badge_name, badge_get_at, badge_owner) VALUES ('secret_phrase_v2', '${obtainedDate}', '${message.author.id}')`, async function(err, results){
                    if (err){
                        throw err
                    } else {
                        var confirmEmbed = new Discord.MessageEmbed()
                            ...
                            .setDescription(`<@${message.author.id}> **vous venez d'acquerir le badge __${bot.badges.get("secret_phrase_v2").name}__**`)
    ...
    if(message.channel.name.startsWith("test-connaissances-de-")){
        ...
        if(message.channel.isTested.testQuestion == message.channel.testTotalQuestions){
            var testEndEmbed = new Discord.MessageEmbed()
                ...
                ${bot.botEmojis.GLOBAL.BULLET} Pour rappel: Il faut minimum **${Math.round((message.channel.testTotalQuestions/3)*2)}/${message.channel.testTotalQuestions}** pour réussir l'examen !
